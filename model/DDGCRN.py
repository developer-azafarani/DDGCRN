...
class DDGCRN(nn.Module):
    ...
    def forward(self, source, i=2):
        ...
        if i == 1:
            init_state1 = self.encoder1.init_hidden(
                source.shape[0]
            )  # [2,64,307,64] 前面是2是因为有两层GRU
            output, _ = self.encoder1(
                source, init_state1, node_embeddings
            )  # B, T, N, hidden
            # output = output[:, -1:, :, :]                                   #B, 1, N, hidden
            output = self.dropout1(output[:, -1:, :, :])
            ...
            init_state2 = self.encoder2.init_hidden(
                source2.shape[0]
            )  # [2,64,307,64] 前面是2是因为有两层GRU
            output2, _ = self.encoder2(
                source2, init_state2, node_embeddings
            )  # B, T, N, hidden

...
def CORR_torch(pred, true, mask_value=None):
    ...
    correlation = ((pred - pred_mean) * (true - true_mean)).mean(dim=dims) / (
        pred_std * true_std
    )
    index = true_std != 0
...
def RRSE_np(pred, true, mask_value=None):
    ...
    mean = true.mean()
    return np.divide(
        np.sqrt(np.sum((pred - true) ** 2)), np.sqrt(np.sum((true - mean) ** 2))
    )
...
def CORR_np(pred, true, mask_value=None):
    ...
    correlation = ((pred - pred_mean) * (true - true_mean)).mean(axis=dims) / (
        pred_std * true_std
    )
    index = true_std != 0
    correlation = (correlation[index]).mean()
